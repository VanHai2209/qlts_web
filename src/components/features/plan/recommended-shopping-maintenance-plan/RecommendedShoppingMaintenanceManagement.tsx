...
import { RecommendedShoppingMaintenanceFilter } from "./components/RecommendedShoppingMaintenanceFilter";
...
const RecommendedShoppingMaintenanceManagement = () => {
  const columns = useMemo<MRT_ColumnDef<IRecommendPlans>[]>(
    () => [
      {
        accessorKey: "id",
        header: "ID",
        Cell: ({ row }) => <span>BTTMKN-{row.original.id}</span>,
      },
      {
  ...
  const handleExportExcel = (data: IRecommendPlans[]) => {
    onGetExport({
      title: "DS BT TM duoc kien nghi",
      worksheetname: "DS BT TM duoc kien nghi",
  ...
  return (
    <TableContextProvider>
      <RecommendedShoppingMaintenanceFilter />

      <ReactTable
        endpointAPI={"/plan/recommended-shopping-maintenance"}
        name={"recommended-shopping-maintenance-plan"}
        columns={columns}
        seeDetail={true}
...
export { RecommendedShoppingMaintenanceManagement };
