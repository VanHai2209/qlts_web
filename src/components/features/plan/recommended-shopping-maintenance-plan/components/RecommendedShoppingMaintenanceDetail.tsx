...
export const RecommendedShoppingMaintenanceDetail = () => {
  const { id } = useParams();
  const queryClient = useQueryClient();
  const [evaluation, setEvaluate] = React.useState("");

  const { data, isLoading, refetch } = useQuery<IRecommendPlans>({
  ...
  const manager = data?.Department.User.find((user) => user.role === "MANAGER");
  ...
  const { mutate } = useMutation({
    mutationFn: async ({ status }: { status: "APPROVED" | "REJECTED" }) => {
      ...
      const res = await request.patch(
        `/plan/recommended-shopping-maintenance/${id}/status`,
        {
          status,
          evaluation,
        }
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["recommended-shopping-maintenance-plan"],
      });
      refetch();
      notifications.show({
        title: "Thành công",
      ...
      {isLoading ? (
        ...
        <Grid>
          ...
          >
            ...
            <Text>Nhà cung cấp: {data?.RecommendPlanAsset?.Supplier.name}</Text>
